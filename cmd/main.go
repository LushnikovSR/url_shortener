...
func main() {
	...
	defer db.Close() // нужно чтобы отпускать коннекты, потому что есть пул соединений, чтобы когда у тебя под гасится чтобы он сразу коннект отпускал и новые поды имели доступ к этим коннектам, потому что в какой-то момент может нехватить коннектов если старые не будут отпускать.
